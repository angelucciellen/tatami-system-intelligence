...
**Version:** 1.28 | **Last Updated:** 2026-06-20 | **Rubric:** v2
**Systems:** 
...
## 18. PHASE B BUILD-OUT TO DO LIST (UPDATED 2026-06-20)

*Master roadmap for all systems, layers, and integrations required to reach Phase B readiness. Updated with Casino Pattern discovery.*

### Five Major Systems (Updated):

| System | Purpose | Status | ETA | Update |
|---|---|---|---|---|
| Webhook Infrastructure | Auto-capture OHLC + engine states | Designed, ready to build | Week 1 | ✅ NQ ready for hybrid mode |
| Parallel Tagging Workflow | GC auto, NQ manual, anomaly review | Designed, execution-ready | Week 1–8 | ✅ NQ hybrid (signal auto, outcome manual) |
| Narrative Audit Engine | Narrative acceptance vs. price | Designed, metrics needed | Week 2–3 | On schedule |
| Behavioral Pressure Meter | Behavioral force classification | Designed, formulas needed | Week 3–4 | On schedule |
| Macro Intelligence Validator | Brief validation vs. COT + IV + data | Designed, API integration needed | Week 2–3 | On schedule |
| **[NEW] Daily Regime Meter** | **Detect WIN/LOSS/MIXED days** | **Designed, ready to build** | **Week 1** | **🆕 Casino Pattern filter** |

### Phase B Filters (Complete Set):
...
- **LED:** Multiply position size by 1.2× (34.3% clean rate)
- **WRONG:** Multiply position size by 0.75× (26.7% stop hunt rate)
...
- **TRAVERSAL:** Base confidence 1.0 (34.4% clean — workhorse)
- **COMPRESSION:** Base confidence 0.8 (limited data)
- **EXPANSION:** Base confidence 0.8 (limited data)
- **FAILED:** Base confidence 0.5 (25.9% stop hunt magnet — avoid or size down)
- **EXHAUSTION:** Base confidence 0.5 (18.8% stop hunt rate)
- **DRIFT:** Base confidence 0.3 (unreliable baseline)
- **Implementation:** State lookup, confidence multiplier
- **Status:** Ready to code

#### Filter 5: Regime Detection (Daily Personality Meter) — NEW
- **By Event 3, measure MAE for first 3 events:**
  ```
  avg_mae_1_3 = (mae_event_1 + mae_event_2 + mae_event_3) / 3
  
  IF avg_mae_1_3 < 0.4:
    regime = "WIN_DAY" (confidence 85%)
    action = "Trust signals, hold through day"
    multiplier = 1.2×
  
  ELIF avg_mae_1_3 >= 1.0:
    regime = "LOSS_DAY" (confidence 90%)
    action = "Widen stops to -1.5 ATR OR re-enter after shake"
    multiplier = 0.8× (reduced confidence)
    stop_adjustment = -1.5 ATR
  
  ELSE:
    regime = "MIXED_DAY" (confidence 70%)
    action = "Use normal parameters, take partial profits early"
    multiplier = 0.9×
  ```
- **Session-specific override:**
  - Tokyo LOSS_DAY: Slightly higher confidence (67% commitment)
  - NY WIN_DAY: Highest confidence (71% clean rate)
  - London: Most balanced (60% clean)
- **Implementation:** Calculate by Event 3, update daily regime status
- **Status:** Ready to code

#### Filter 6: MAE-Based Position Management (NEW)
- **WIN_DAY signals:** Use standard -1.0 ATR stop
- **LOSS_DAY signals:** 
  - Option A: Widen stop to -1.5 ATR (catch more upside, accept risk)
  - Option B: Enter, expect shake to -1.8 ATR, re-enter after
  - Option C: Skip signal (preserve capital on choppy days)
- **MIXED_DAY signals:** Use standard -1.0 ATR, take partial profit at 0.5 ATR
- **Implementation:** Stop recommendation engine, optional auto-adjustment
- **Status:** Ready to code

### Integration Points (Updated):

All five systems + new regime meter feed into enhanced webhook payload:
```json
{
  "time": "...",
  "event_id": "E###",
...
This complete payload feeds Phase C2 analyzer for conditional rule generation + regime-adjusted execution.

### Phase B Success Criteria (Week 8 — Updated):

- 1,000–1,200 GC events (webhook)
- 300–400 NQ events (hybrid webhook + manual outcome)
- 15–20 anomalies resolved
- **6 filters locked (was 3–5, now includes regime meter)**
- LED baseline GC ≥60%, NQ ≥55%
- **Regime detection validated: 85%+ accuracy**
- Confidence level 75%+ ready to commit
- **Stop hunt prediction accuracy 80%+**

### Timeline (Updated):

- **Week 1:** Webhook setup for NQ (hybrid mode) + regime meter design
- **Weeks 2–4:** Narrative + Pressure + Validator frameworks + regime meter integration
- **Weeks 5–7:** All systems integration + regime testing
- **Week 8:** Phase B readiness decision + go/no-go on regime-based execution

### New Implementation Tasks (Casino Pattern):

#### Task 1: Regime Meter Backend (3 days)
- [ ] Create regime detection function in Python
- [ ] Calculate daily MAE averages
- [ ] Classify days as WIN/LOSS/MIXED
- [ ] Store regime status in database
- [ ] Endpoint: `/api/regime/daily/{date}/{session}`

#### Task 2: Outcome Input UI (2 days)
- [ ] Build web form for outcome entry (bars 4-6)
- [ ] Fields: event_id, outcome_direction, mae_atr, notes
- [ ] Form submits and triggers auto-classification
- [ ] Dashboard shows pending events queue

#### Task 3: Auto-Classification (2 days)
- [ ] Implement classification rules (CLEAN vs STOPHUNT vs OPPOSITE)
- [ ] Apply state/tag/quality multipliers
- [ ] Update daily regime after each outcome
- [ ] Generate stop recommendations per regime

#### Task 4: Casino Meter Dashboard Widget (3 days)
- [ ] Display current day regime (WIN/LOSS/MIXED)
- [ ] Show regime confidence score
- [ ] List recommended stop levels by regime
- [ ] Show regime persistence (is today likely same as yesterday?)
- [ ] Color code: Green (WIN), Red (LOSS), Gray (MIXED)

#### Task 5: Historical Regime Analysis (2 days)
- [ ] Tag all Batch 5 events with regime classifications
- [ ] Generate regime persistence statistics
- [ ] Build session-specific regime profiles
- [ ] Document regime patterns by instrument (GC vs NQ)

#### Task 6: Validation & Live Testing (ongoing)
- [ ] Parallel run: manual vs. hybrid tagging for 1 week
- [ ] Measure regime detection accuracy
- [ ] Compare actual outcomes vs. predicted regime
- [ ] Refine MAE thresholds if needed
- [ ] Lock final thresholds in GT

---

**Critical Path:** Regime Meter > Outcome Input > Auto-Classification > Dashboard (1 week total)
