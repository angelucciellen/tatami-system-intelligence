...
**Version:** 1.21 | **Last Updated:** 2026-06-13 | **Rubric:** v2
...
| Instruments | GC 5m (active) · NQ 5m (active) |
| Tagging source | TradingView Bar Replay |
| HTFs | 60m · 240m |
| GC 5m | Batch 1 locked (events 1–50) · Batch 2 locked (events 51–100) · Batch 3 locked (events 101–150) · Batch 4 next |
| NQ 5m | RESTARTED FROM E001 (2026-06-08) · Full framework active · Previous data discarded |
| Combined main total | GC: 150 locked · NQ: 0 (clean restart from E001) / 350–400 target per instrument |
...
**⚠ NQ DATASET RESTARTED 2026-06-08 — Previous data discarded entirely.**

Previous ~200 events (Dec 31 2025 – Jan 9 2026) discarded. Reason: memory zones off throughout — zone_density field systematically invalid. Data not archived. Clean start.

**Full framework now active from E001:**
...
| Batch | Events | Week(s) sampled | Regime notes |
|---|---|---|---|
| Batch 1 | E001–E050 | TBD — second full trading week | First batch. Full framework active. No filtered batch — methodology proven from GC. Comparison baseline = GC B2 (unfiltered). |
| Batch 2 | E051–E100 | TBD | — |
| Batch 3 | E101–E150 | TBD | — |
| Batch 4–8 | E151–E400 | TBD | — |
...
**What's next:** Complete GC 5m tagging (Batches 4–8, second week per month) → build NQ dataset from E001 with full framework (The Mechanism + vX.14 Trap Resonance) → Phase B strategy harness when both datasets reach 350–400 events.
...
| `trap-resonance: YES · trap-type: [WICK/DOT]` | `trap-resonance: YES · trap-type: WICK` | vX.14 TRAP fires on signal bar or within 1 bar before it. Positive cases only — do not flag absence. Optional addition: `zone-type: [red/brown/blue]` when zone type is visible. *(Event Detection Layer)* |
...
| `channel-50: YES · levels: [lower]-[upper] · level: [price]` | `channel-50: YES · levels: 28967-29402 · level: 29184` | Signal bar fires within 0.5 ATR of the 50% midpoint between two Tier 1 macro yellow levels. The 50% is the equilibrium point of the channel between those two structural stocks. Positive cases only. *(Location Layer — fractal midpoint)* |
...
| Channel | 50% Midpoint |
|---|---|
| 28,967–29,402 | ~29,184 |
| 28,542–28,967 | ~28,754 |
| 29,402–30,289 | ~29,845 |
| 26,225–28,542 | ~27,383 |
| 24,674–26,225 | ~25,449 |
...
- Channel 50% midpoint: equilibrium zone between two Tier 1 levels — captured via `channel-50` note

**Fractal scale invariance (added v1.21 · 2026-06-13):**
The same behavioral pattern repeats at every scale. The macro structural levels (yellow lines) are the weekly/monthly expression of the same rail-to-rail traversal the engine detects at the 5m level. Price travels from one yellow level to the next the same way price travels from one channel rail to the other on the 5m chart — because the underlying feedback loop mechanics are identical regardless of timeframe. This is scale invariance — a structural property of complex adaptive systems (Meadows, 2008).

Multi-scale confluence: when the macro level (yellow line), the intermediate rails (60m/240m), and the 5m engine rails all agree on the same location simultaneously — this represents the highest structural density in the framework. Behavioral outcomes at these confluence points are expected to be materially more predictable than at any single-scale location. The dataset will quantify this in Phase B.
...
**tagging note convention:** `trap-resonance: YES · trap-type: [WICK/DOT]` in notes field.
